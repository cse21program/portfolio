const HOURLY = /\/\s*hour/i;

export function parsePriceCents(label: string) {
  const trimmed = label.trim();
  if (!trimmed || HOURLY.test(trimmed)) {
    return null;
  }
...
export function lineCents(unitCents: number, quantity: number) {
  return Math.max(0, unitCents) * Math.max(1, quantity);
}
